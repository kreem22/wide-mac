...
> **CRITICAL: html2pptx is a JavaScript library, NOT a CLI tool. It has NO command-line interface.**
> - ❌ `html2pptx -c config.json` — will NOT work (no such command exists)
> - ❌ `npx @ant/html2pptx ...` — will NOT work (no CLI binary in the package)
> - ❌ Creating a `config.json` for html2pptx — not supported
> - ❌ Writing a Python script instead — use the JavaScript workflow shown below
> - ✅ Write a `.js` script with `import { html2pptx } from '@ant/html2pptx'` and run with `node script.js`

> **Playwright and Chromium are pre-installed.** Do NOT run `playwright install` — it will hang. The html2pptx library handles browser operations internally.

**Important**: The @ant/html2pptx package is pre-installed globally in the Docker image.

**You must use ES Modules (import) syntax:**

```javascript
// Use ES module imports — CommonJS require() does NOT work with html2pptx
...
**Note**:
- CommonJS `require()` does NOT work due to ES-only dependencies
- Create `package.json` with `{"type": "module"}` in the working directory before running
- All packages (`pptxgenjs`, `@ant/html2pptx`, `react`) are available by package name
...
**Your script MUST follow the following example.** Use ES module `import` syntax (NOT CommonJS `require`). Create `package.json` with `{"type": "module"}` before running.
...
All packages are installed in `/home/node_modules` (or symlinked there) and are resolved automatically by Node.js ES module resolution.
